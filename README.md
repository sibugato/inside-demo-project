...
- К нашим требованиям добавляется среда разработки и установленный `Maven` для 
сборки проекта. Сначала клонируйте себе этот репозиторий. 
 Прежде чем продолжить убедитесь 
что у вас установлена JDK 11 версии или выше. Для этого выполните в терминале среды 
...
После этого подготовьте .jar файл сервиса - просто выполните эту команду: `mvn clean install`. Будет выполнена сборка проекта и последующее создание необходимого нам артефакта. Теперь объединяем его с образом
